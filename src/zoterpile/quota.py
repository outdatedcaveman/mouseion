...
The quota state is persisted to ``~/.cache/zoterpile/quota.json`` so limits
are respected across multiple process invocations within the same day.

Usage
-----
    from zoterpile.quota import get_default_quota_manager
...
_DEFAULT_QUOTA_FILE = Path.home() / ".cache" / "zoterpile" / "quota.json"
...
DEFAULT_PROVIDER_LIMITS: Dict[str, ProviderLimits] = {
    "crossref":         ProviderLimits(requests_per_minute=50,  requests_per_hour=3_000,  requests_per_day=100_000),
    "openalex":         ProviderLimits(requests_per_minute=100, requests_per_hour=6_000,  requests_per_day=100_000),
    "semantic_scholar": ProviderLimits(requests_per_minute=60,  requests_per_hour=3_600,  requests_per_day=50_000,  min_interval_seconds=1.05),
    "pubmed":           ProviderLimits(requests_per_minute=10,  requests_per_hour=600,    requests_per_day=10_000),
    "dblp":             ProviderLimits(requests_per_minute=10,  requests_per_hour=500,    requests_per_day=10_000),
    "arxiv":            ProviderLimits(requests_per_minute=3,   requests_per_hour=180,    requests_per_day=3_000,   min_interval_seconds=0.34),
}


class QuotaExceeded(Exception):
    """Raised when a provider's daily quota has been exhausted."""
    def __init__(self, provider: str, window: str, limit: int) -> None:
