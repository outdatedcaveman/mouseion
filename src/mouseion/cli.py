"""
mouseion CLI — comprehensive reference lookup, enrichment and sync.
...
--------
  ...
  mouseion add "10.1038/nature12373"
  mouseion add "https://arxiv.org/abs/1706.03762"
  mouseion add "Attention Is All You Need"
  mouseion add bookmarks.html
  mouseion add refs.bib --tags "AI,to-read"
  mouseion add "10.1038/s41586-023-01, 10.1126/science.abc1234"

  # Search
  mouseion search "transformer attention"
  mouseion search "" --tag preprint --year-from 2022

  # Export
  mouseion export --format bibtex -o all_refs.bib
  mouseion export --tag ML --format ris -o ml_refs.ris

  # Sync to external tools
  mouseion sync zotero
  mouseion sync notion
  mouseion sync obsidian
  mouseion sync all

  # PDF fetching
  mouseion fetch-pdfs --limit 100
"""
...
@click.version_option(package_name="mouseion")
def main():
    """
    mouseion — universal reference enrichment and management.
...
def pull_sync(source, full, do_enrich, collection_id, dry_run):
    ...
    Zotero example:
      mouseion pull-sync zotero
      mouseion pull-sync zotero --full --collection ABC123XY

    Notion example:
      mouseion pull-sync notion
    """
    if source == "zotero":
...
def dedup_all(max_merges: int, yes: bool):
    """Run all safe bulk dedup rules with an automatic restore point."""
    if not yes:
        click.confirm(
            "This will merge/delete duplicate rows after creating a restore point. Continue?",
            abort=True,
        )
...
def semantic_search(query, n, year_from, year_to, ref_type, oa, fmt, output):
    ...
    if not idx.is_available():
        console.print(
            ...
            "Then rebuild the index:  mouseion index-semantic"
        )
        raise SystemExit(1)
    ...
    if idx.count() == 0:
        console.print(
            ...
            "Build it with:  mouseion index-semantic"
        )
        return
    ...
    if rebuild:
        ...
        try:
            ...
            path = cfg.semantic_index_path or str(
                Path.home() / ".local" / "share" / "mouseion" / "semantic"
            )
            ...
            import mouseion.semantic as _sem_mod
