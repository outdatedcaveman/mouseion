...
class OpenAlexProvider(BaseProvider):
    ...
    priority = 2
    _max_concurrent = 3
    _min_interval = 0.5  # ~2 req/s — search endpoint is stricter than filter
    ...
    def __init__(self, email: Optional[str] = None, api_key: Optional[str] = None) -> None:
        if not email or not api_key:
            try:
                ...
                cfg = get_config()
                email = email or cfg.openalex_email
    ...
    async def lookup_by_doi(
        ...
        resp = await self._get(client, url, params=self._auth_params())
        if resp is None:
            return None
