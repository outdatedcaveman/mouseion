...
def _clean_import_title(t: str) -> str:
    """Strip common noise from bookmark/academia.edu titles."""
    t = re.sub(r'^\(\d+\)\s*', '', t)  # "(7) Title" → "Title"
    t = re.sub(r'^\[?\s*(?:PDF|FULL\s*TEXT|HTML)\s*\]?\s*[|:\-–—]\s*', '', t, flags=re.IGNORECASE)
    t = re.sub(r'\s*[\-–—|]\s*(?:Academia\.edu|ResearchGate|Google Scholar).*$', '', t, flags=re.IGNORECASE)
    return t.strip()
...
def _url_to_seed(url: str, title: str, folders: List[str]) -> Reference:
    ref = Reference()
    ref.url = url
    ref.title = _clean_import_title(title) or title.strip() or None
