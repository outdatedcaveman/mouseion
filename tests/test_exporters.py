...
from mouseion.exporters.bibtex import to_bibtex_string, export_bibtex_file
from mouseion.exporters.ris import to_ris_string, export_ris_file
from mouseion.exporters.markdown import to_markdown_string, export_markdown_file
from mouseion.exporters.zotero_rdf import to_zotero_rdf_string
...
class TestZoteroRDF:
    ...
    def test_urn_fallback_uri(self):
        """Refs without DOI or URL get a urn:mouseion: URI."""
        ref = Reference(title="No ID Ref", ref_type=RefType.JOURNAL, year=2020)
        rdf = to_zotero_rdf_string([ref])
        assert 'rdf:about="urn:mouseion:' in rdf
